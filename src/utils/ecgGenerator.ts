...
// Стимульований ритм — стимулюючий спайк + широкий QRS
const pacedBeat = (phase: number): number => {
  let v = 0;
  v += g(phase, 0.15, 0.003, 1.5);   // стимулюючий спайк
  v += g(phase, 0.20, 0.022, 0.85);  // стимульований QRS
  v += g(phase, 0.28, 0.018, -0.30); // S-хвиля
  v += g(phase, 0.45, 0.050, 0.20);  // T-хвиля
  return v + noise(0.015);
};
  ...
  for (let i = 0; i < samples; i++) {
    ...
    switch (rhythm) {
      ...
      case 'asystole':
        ...
        break;
      case 'paced': {
        const rr = 60 / 60; // стимуляція 60/хв
        value = pacedBeat((t % rr) / rr);
        break;
      }
    }
    buffer.push(value);
...
export const RHYTHM_LABELS: Record<RhythmType, string> = {
  ...
  asystole:            'Асистолія',
  paced:               'Стимульований ритм (Paced)',
};
