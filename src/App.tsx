...
export default function App() {
  ...
  if (!agreed) {
    return (
      <div className="fixed inset-0 flex items-center justify-center p-4" style={{ background: '#000810' }}>
        <motion.div initial={{ opacity:0, scale:0.95 }} animate={{ opacity:1, scale:1 }}
          className="w-full max-w-lg rounded-xl shadow-2xl overflow-hidden"
          style={{ background:'#000f1e', border:'1px solid #0d2035' }}>
          <div className="px-6 py-4 border-b flex items-center justify-between" style={{ borderColor:'#0d2035', background:'#00060f' }}>
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-bold text-white text-sm uppercase tracking-widest">{t('disclaimerTitle', lang)}</h1>
                <p className="text-[10px]" style={{ color:'#446688' }}>{t('disclaimerSubtitle', lang)}</p>
              </div>
            </div>
          ...
          <div className="px-6 py-5 space-y-4 text-sm leading-relaxed" style={{ color:'#aaccdd' }}>
            <p>{t('disclaimerBody1', lang).split('не можуть бути підставою')[0]}{lang === 'ua' && <><strong className="text-red-400">не можуть бути підставою для встановлення діагнозу або призначення лікування реальним пацієнтам</strong>.</>}{lang === 'en' && <><strong className="text-red-400">cannot serve as a basis for diagnosis or treatment of real patients</strong>.</>}</p>
            <p>{t('disclaimerBody2', lang)}</p>
            <p className="text-xs" style={{ color:'#446688' }}>{t('disclaimerBody3', lang)}</p>
          </div>
          <div className="px-6 py-4 border-t" style={{ borderColor:'#0d2035', background:'#00060f' }}>
            <button onClick={() => { sessionStorage.setItem('icu_agreed','1'); setAgreed(true); }}
              className="w-full py-2.5 rounded-lg font-bold text-sm"
              style={{ background:'#004422', color:'#44ff88', border:'1px solid #006633' }}>
              {t('disclaimerBtn', lang)}
            </button>
          </div>
        </motion.div>
      </div>
      ...
      <div className="flex h-full w-full overflow-hidden">
        ...
        <aside className="w-48 lg:w-60 border-r border-gray-800 flex flex-col bg-[#0F1219] shrink-0">
          <div className="p-2 text-[10px] font-bold text-gray-500 border-b border-gray-800 uppercase tracking-widest bg-[#11141D] flex justify-between items-center">
            {lang === 'ua' ? 'Відділення' : 'Ward Overview'}
            <button 
          ...
          <div className="p-2 border-t border-gray-800 bg-[#11141D] text-[9px] text-gray-600 font-mono flex justify-between">
            <span>{lang === 'ua' ? 'Моніторів' : 'Active Monitors'}: {patients.length}</span>
