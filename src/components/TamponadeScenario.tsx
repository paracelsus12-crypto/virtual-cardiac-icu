  ...
  const handleAction = (actionId: string) => {
    ...
    if (action.requiresConsultant && !consultantCalled) {
      const msg = isOpen
        ...
        : lang === 'ua' ? '❌ Потрібен хірург. Спочатку повідомте старшого колегу.' : '❌ Surgeon required. Call senior colleague first.';
      ...
      <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}
        ...
        <div className="w-full max-w-lg bg-[#0d0f14] border border-green-800/50 rounded-xl p-6 shadow-2xl">
          <div className="flex items-center gap-3 mb-1">
            ...
            <div>
              <h2 className="text-white font-bold text-base">{t("tampDebrief", lang as Lang)}</h2>
      ...
      <div className="shrink-0 px-4 py-2 border-b flex items-center justify-between"
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <span className="font-bold text-white text-sm">{t("tampTitle", lang as Lang)}</span>
            <span className="ml-2 text-xs font-bold" style={{ color: phaseColor }}>
              {PHASE_LABELS[state.phase][lang][lang]}
            </span>
      ...
      <div className="flex-1 flex overflow-hidden min-h-0">
        ...
        <div className="flex-1 flex flex-col p-3 gap-2 overflow-y-auto min-w-0">
          ...
          <WaveformCanvas buffer={abpBuffer} color="#ef4444" fillColor="rgba(239,68,68,0.07)"
            label={lang === 'ua' ? `АТ — Парадоксальний пульс: ${state.paradoxicalPulse} мм рт.ст.${state.paradoxicalPulse >= 10 ? ' ⚠️' : ''}` : `ABP — Paradoxical Pulse: ${state.paradoxicalPulse} mmHg${state.paradoxicalPulse >= 10 ? ' ⚠️' : ''}`}
          ...
          <div className="bg-[#11141D] rounded border border-gray-800 p-3">
            <div className="text-[9px] font-bold text-gray-500 uppercase tracking-widest mb-2">{t("tampBeck", lang as Lang)}</div>
            <div className="grid grid-cols-3 gap-2">
              <BeckSign active={state.systolicBP < 90} label={t("tampHypotension", lang as Lang)} value={`${state.systolicBP} мм`} />
              <BeckSign active={state.cvp > 12}        label={t("tampCVPHigh", lang as Lang)}      value={`${state.cvp} мм`} />
              <BeckSign active={state.ecgVoltage < 0.6} label={t("tampECGLow", lang as Lang)} value={`${Math.round(state.ecgVoltage*100)}%`} />
            </div>
          </div>
          ...
          {isOpen && (
            <div className={`rounded border p-3 ${state.drainBlocked ? 'bg-red-950/20 border-red-800/40' : 'bg-[#11141D] border-gray-800'}`}>
              <div className="text-[9px] font-bold text-gray-500 uppercase tracking-widest mb-1.5">{t("tampDrain", lang as Lang)}</div>
              <div className="flex items-center justify-between">
                <div>
                  <span className={`text-sm font-bold font-mono ${state.drainBlocked ? 'text-red-400' : 'text-green-400'}`}>
                    {state.drainBlocked ? (lang === 'ua' ? '⛔ ЗАБЛОКОВАНИЙ' : '⛔ BLOCKED') : `${state.drainOutput} ${lang === 'ua' ? 'мл/год' : 'ml/hr'}`}
                  </span>
                  {state.drainBlocked && (
                ...
                {!state.drainBlocked && (
                  <span className="text-[10px] text-gray-500">{lang === 'ua' ? 'норма <100 мл/год' : 'normal <100 ml/hr'}</span>
                )}
              </div>
            </div>
          )}
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="font-bold text-xs leading-tight">{lang === 'ua' ? action.labelUa : action.labelEn}</span>
                  ...
                  <p className="text-[9px] opacity-55 mt-0.5 pl-5 leading-snug">{lang === 'ua' ? action.descUa : action.descEn}</p>
                  {locked && (
                    <p className="text-[9px] text-yellow-400 mt-0.5 pl-5">
                      {needsEcho ? (lang === 'ua' ? '🔒 Потрібне ехо' : '🔒 Echo required') : (lang === 'ua' ? '🔒 Потрібен хірург' : '🔒 Surgeon required')}
                    </p>
                  )}
                </button>
              );
...
const BeckSign = ({ active, label, value }: { active: boolean; label: string; value: string }) => (
  <div className={`p-2 rounded border text-center transition-colors ${active ? 'bg-red-950/30 border-red-600/50' : 'bg-gray-900/50 border-gray-800'}`}>
    ...
    {active && <div className="text-[8px] text-red-500 mt-0.5">⚠ ABNORMAL</div>}
  </div>
);
