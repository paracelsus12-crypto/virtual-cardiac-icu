...
const HypotensionScenario: React.FC<HypotensionScenarioProps> = ({
  ...
  const handleTest = (testId: string) => {
    ...
    setLog(prev => [{ time: timeMin, message: `${test.icon} ${lang === 'ua' ? test.label : test.labelEn ?? test.label}: ${result}`, type: 'info' }, ...prev]);
      ...
      <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }}
        ...
        <div className="w-full max-w-lg bg-[#0d0f14] border border-green-800/50 rounded-xl p-6 shadow-2xl overflow-y-auto max-h-[90vh]">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h2 className="text-white font-bold">{t("hypoDebrief", lang as Lang)}</h2>
          ...
          <div className="bg-gray-900 rounded p-3 mb-4 space-y-1 text-[11px]">
            <p className="font-bold text-white text-xs mb-1">{t("hypoKeyPoints", lang as Lang)}:</p>
            {debrief.keyPoints.map((p, i) => <p key={i} className="text-gray-300">• {p}</p>)}
            {penaltyActions.length > 0 && (
              <div className="mt-2 pt-2 border-t border-gray-700">
                <p className="text-red-400 font-bold text-[10px]">{t("hypoPenalty", lang as Lang)}:</p>
                ...
                return (
                  <button key={test.id} onClick={() => handleTest(test.id)}
                    ...
                    <div className="flex items-center gap-1.5">
                      <span>{test.icon}</span>
                      <span className="font-medium">{lang === 'ua' ? test.label : test.labelEn ?? test.label}</span>
                      {done && <span className="ml-auto text-green-400 text-[10px]">✓</span>}
                      {cd > 0 && <span className="ml-auto font-mono text-[10px]">{cd}s</span>}
                    </div>
                    <p className="text-[9px] text-gray-500 mt-0.5 pl-5">{lang === 'ua' ? test.description : test.descEn ?? test.description}</p>
                  </button>
                );
              })}
            </div>
                ...
                return (
                  <button key={action.id} onClick={() => handleTreatment(action.id)}
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="font-bold text-xs">{lang === 'ua' ? action.label : action.labelEn ?? action.label}</span>
                      {cd > 0 && <span className="ml-auto font-mono text-[10px] opacity-60">{cd}s</span>}
                    </div>
                    <p className="text-[9px] opacity-55 mt-0.5 pl-5">{lang === 'ua' ? action.description : action.descEn ?? action.description}</p>
                  </button>
                );
              })}
            </div>
