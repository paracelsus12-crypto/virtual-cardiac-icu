...
const getRhythmLabel = (r: RhythmType, l: Lang): string => {
  const labels: Record<string, { ua: string; en: string }> = {
    sinus:              { ua: 'Синусовий ритм',               en: 'Sinus Rhythm' },
    afib:               { ua: 'Фібриляція передсердь',        en: 'Atrial Fibrillation' },
    av_block_1:         { ua: 'АВ-блокада I ст.',             en: 'AV Block I°' },
    av_block_2_mobitz1: { ua: 'АВ-блокада II ст. (Мобітц I)', en: 'AV Block II° (Mobitz I)' },
    av_block_2_mobitz2: { ua: 'АВ-блокада II ст. (Мобітц II)',en: 'AV Block II° (Mobitz II)' },
    av_block_3:         { ua: 'АВ-блокада III ст.',           en: 'Complete AV Block' },
    vtach:              { ua: 'Шлуночкова тахікардія',        en: 'Ventricular Tachycardia' },
    vfib:               { ua: 'Фібриляція шлуночків',         en: 'Ventricular Fibrillation' },
    asystole:           { ua: 'Асистолія',                    en: 'Asystole' },
    paced:              { ua: 'Стимульований ритм',           en: 'Paced Rhythm' },
  };
  return labels[r]?.[l] ?? r;
};

const RHYTHM_OPTIONS: { value: RhythmType; color: string }[] = [
  { value: 'sinus',              color: 'text-green-400' },
  { value: 'afib',               color: 'text-yellow-400' },
  { value: 'av_block_1',         color: 'text-blue-300' },
  { value: 'av_block_2_mobitz1', color: 'text-yellow-500' },
  { value: 'av_block_2_mobitz2', color: 'text-orange-400' },
  { value: 'av_block_3',         color: 'text-orange-500' },
  { value: 'vtach',              color: 'text-red-400' },
  { value: 'vfib',               color: 'text-red-600' },
  { value: 'asystole',           color: 'text-gray-400' },
  { value: 'paced',              color: 'text-cyan-400' },
];
      ...
      <div className="shrink-0 px-3 py-1.5 flex items-center justify-between border-b"
        ...
        <div className="flex items-center gap-4">
          <div>
            ...
            <span className="ml-2 text-[10px] font-mono" style={{ color:'#4488aa' }}>
              {t('bed', lang as Lang)} {patient.bedNumber}
            </span>
          </div>
          <div className="text-[10px]" style={{ color:'#4488aa' }}>
            {patient.age} р. · {patient.gender==='M' ? t('male', lang as Lang) : t('female', lang as Lang)}
          </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={handleReset}
            ...
            title={t("reset", lang as Lang)}>
            ↺ {t("reset", lang as Lang)}
          </button>
          ...
          {isCritical && (
            <button onClick={() => setShowResuscitation(true)}
              ...
              <Zap size={11}/> {t('btnResuscitation', lang as Lang)}
            </button>
          )}
          <button onClick={() => setShowTamponade(true)}
            ...
            <Droplets size={11}/> {t('btnTamponade', lang as Lang)}
          </button>
          ...
          <button onClick={() => { resumeAudioContext(); setShowRhythmSelector(s=>!s); }}
            ...
            <Settings size={11}/> {lang === 'ua' ? 'РИТМ' : 'RHYTHM'}
          </button>
        ...
        {showRhythmSelector && (
          <motion.div initial={{opacity:0,y:-6}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-6}}
            ...
            <div className="col-span-2 text-[9px] font-bold uppercase px-1 pb-1.5 mb-1"
              ...
              lang === 'ua' ? 'Навчальний сценарій — вибір ритму' : 'Training scenario — select rhythm'
            </div>
            {RHYTHM_OPTIONS.map(opt => (
              <button key={opt.value} onClick={() => handleRhythmChange(opt.value)}
                ...
                {getRhythmLabel(opt.value, lang as Lang)}
              </button>
            ))}
          </motion.div>
        )}
      ...
      <div className="shrink-0 px-3 py-1 flex items-center gap-4 border-t text-[9px]"
        ...
        {currentVitals.spo2 < patient.thresholds.spo2Min && (
          <span className="font-bold animate-pulse" style={{ color:'#ff6644' }}>
            {t('spo2Low', lang as Lang)}: {currentVitals.spo2}%
          </span>
        )}
        {currentVitals.heartRate > patient.thresholds.hrMax && (
          <span className="font-bold" style={{ color:'#ffaa00' }}>
            {t('tachycardia', lang as Lang)}: {currentVitals.heartRate} {lang==='ua'?'уд/хв':'bpm'}
          </span>
        )}
        <span className="ml-auto" style={{ color:'#1a3a4a' }}>
          V-ICU · {lang === 'ua' ? 'Кардіореанімація' : 'Cardiac ICU'}
        </span>
      </div>
